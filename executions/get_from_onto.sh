...
if [ "$1" == "1" ]; then
	...
	get_disease_mondo.rb -i $list_path/$term.mondo -k 'Orphanet:[0-9]*' -s -f $ontology_file -o $list_path/$term.dict
...
if [ "$1" == "3" ]; then
	...
	grep -w $go $temp_files'/goa_human.gaf' | cut -f 3 | sort -u > $list_path/go_gene.lst
	...
	get_disease_mondo.rb -i $list_path'/go_mondo.txt' -k 'Orphanet:[0-9]*' -s -f $ontology_file -o $list_path/go_query.dict
